...
export function parsePurchaseEvent(content: Buffer): PurchaseEvent {
  ...
  if (
    Object.keys(event).sort().join(",") !==
      "deductionTotalMinor,eventId,eventType,requestId,timestamp,transactionId" ||
    event.eventType !== PURCHASE_QUEUE ||
    !["eventId", "requestId", "transactionId", "timestamp"].every(
      (key) =>
        typeof event[key] === "string" &&
        (event[key] as string).length > 0 &&
        (event[key] as string).length <= 128,
    ) ||
    event.eventId !== event.transactionId ||
    !Number.isFinite(Date.parse(event.timestamp as string)) ||
    !Number.isSafeInteger(event.deductionTotalMinor) ||
    (event.deductionTotalMinor as number) <= 0
  ) {
    throw new Error("Invalid purchase event");
  }
...
export async function consumePurchaseMessage(
  message: ConsumeMessage,
  channel: Pick<Channel, "ack" | "reject">,
  logger: AppLogger,
): Promise<void> {
  let event: PurchaseEvent;
  try {
    event = parsePurchaseEvent(message.content);
  } catch {
...
async function closeConnection(connection: ChannelModel): Promise<void> {
  try {
    await bounded(connection.close(), 1000);
  } catch {
    ...
    (
      connection as unknown as { connection?: { stream?: { destroy(): void } } }
...
export function startQueue(
  ...
  const pending = db.prepare(
    "SELECT id, request_id, created_at, deduction_total_minor FROM transactions WHERE event_published_at IS NULL ORDER BY created_at, id LIMIT 50",
  );
  const markPublished = db.prepare(
    "UPDATE transactions SET event_published_at = ? WHERE id = ? AND event_published_at IS NULL",
  );
  let stopped = false;
  let timer: NodeJS.Timeout | undefined;
  let running: Promise<void> | undefined;
  let state:
    | { connection: ChannelModel; publisher: ConfirmChannel; consumer: Channel }
    | undefined;
  let nextConnect = 0;
  ...
  async function ensureConnection(): Promise<NonNullable<typeof state>> {
    ...
    if (!brokerUrl.searchParams.has("heartbeat"))
      brokerUrl.searchParams.set("heartbeat", "10");
    const connecting = connect(brokerUrl.toString(), {
      timeout: timeoutMs,
    }).then((connection) => {
      connection.on("error", () => {
      ...
      if (abandoned || stopped) {
        void closeConnection(connection);
        throw new Error("Connection abandoned");
      }
      return connection;
    });
    ...
    try {
      ...
      await bounded(
        publisher.assertQueue(PURCHASE_QUEUE, { durable: true }),
        timeoutMs,
      );
      ...
      await bounded(
        consumer.consume(
          ...
          (message) => {
            ...
            const work = consumePurchaseMessage(message, consumer, logger)
              .catch(() => {
      ...
      abandoned = true;
      if (connection) {
        if (state?.connection === connection) state = undefined;
      ...
      await bounded(
        new Promise<void>((resolve, reject) => {
          publisher.sendToQueue(
            ...
            (error) => {
              if (error || returned)
                reject(error ?? new Error("Unroutable event"));
              else resolve();
            },
          );
        }),
  ...
  async function iteration(): Promise<void> {
    ...
    try {
      ...
      for (const row of pending.all() as PendingRow[]) {
        ...
        logger.info("purchase.completed.published", {
          transactionId: row.id,
          eventId: row.id,
          requestId: row.request_id,
        });
  ...
  function wake(): void {
    ...
    running = iteration().finally(() => {
      running = undefined;
      if (!stopped) {
        timer = setTimeout(wake, intervalMs);
        timer.unref();
      }
    });
  }
  ...
  return {
    ...
    async stop() {
      ...
      if (running)
        await bounded(running, timeoutMs * 5 + 1500).catch(() => undefined);
      await bounded(Promise.allSettled(activeConsumers), 1000).catch(
        () => undefined,
      );
    },
  };
