...
export function authRoutes(db: Database.Database, config: Config) {
  ...
  const context = (dealerId: string, username: string) => {
    const wallet = db
      .prepare("SELECT id FROM wallets WHERE dealer_id = ?")
      .get(dealerId) as { id: number } | undefined;
    if (!wallet)
      throw new AppError(
        404,
        "WALLET_NOT_FOUND",
  ...
  router.get("/me", requireAuth, (req, res) =>
    res.json({ data: context(req.session.dealerId!, req.session.username!) }),
  );
