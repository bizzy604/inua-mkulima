...
const cartShape = {
  items: z
    .array(
      z
        .object({
          ...
          expectedUnitPriceMinor: safeMinor.refine(
            (value) => value > 0,
    ...
    .min(1)
    .max(50),
  expectedDeductionTotalMinor: safeMinor.refine(
    (value) => value > 0,
    "The total deduction must be positive.",
  ),
};
function uniqueProducts(
  cart: { items: { productId: number }[] },
...
export const paymentSchema = z
  .object({
    ...
    verificationCode: z
      .string()
      .regex(/^\d{6}$/, "Enter the six-digit demo verification code."),
  })
  .strict()
  .superRefine(uniqueProducts);
