...
export function createApp(options: {
    ...
    res.on("finish", () => {
      ...
      try {
        logger.info("http.request", {
          ...
          ...(res.locals.transactionId
            ? {
                transactionId: res.locals.transactionId,
                outcome: res.locals.purchaseOutcome,
              }
            : {}),
        });
      } catch {
        /* Logging availability cannot alter an HTTP response. */
      }
    });
    next();
  });
  ...
  app.use(
    "/api/transactions",
    requireAuth,
    transactionRoutes(db, config, options.onPurchase ?? (() => {})),
  );
  app.use("/api", (_req, _res, next) =>
    next(new AppError(404, "NOT_FOUND", "The endpoint was not found.")),
  );
  const frontend = resolve(projectRoot, "frontend/dist");
  if (existsSync(resolve(frontend, "index.html"))) {
    app.use(express.static(frontend));
    app.get(["/", "/login", "/products", "/summary"], (_req, res) =>
      res.sendFile(resolve(frontend, "index.html")),
    );
  }
  app.use((_req, _res, next) =>
    next(new AppError(404, "NOT_FOUND", "The page was not found.")),
  );
