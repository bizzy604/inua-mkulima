...
async function main() {
  ...
  const {app,close} = createApp({config,db,sessionDb,logger,onPurchase:queue.wake});
  ...
  server.once('error',error => {
    logger.error('server.failed',{code:(error as NodeJS.ErrnoException).code ?? 'UNKNOWN'});
    process.exitCode = 1;
    void shutdown();
  });
}
main().catch(() => {
  ...
  console.error('Backend startup failed. Check .env configuration, database permissions, and migrations.');
  process.exitCode = 1;
});
