...
export function seed(db: Database.Database, dealerId = 'demo-dealer'): void {
  db.transaction(() => {
    ...
    const insert = db.prepare('INSERT INTO products(id, name, price_minor, created_at, updated_at) VALUES (?, ?, ?, ?, ?) ON CONFLICT(id) DO NOTHING');
    const products = [
    ...
    for (const [id, name, price] of products) insert.run(id, name, price, now, now);
    db.prepare(`INSERT INTO wallets(dealer_id, farmer_name, farmer_reference, farmer_phone, name, balance_minor, updated_at)
...
export function dumpDatabase(db: Database.Database): string {
  return db.transaction(() => {
    const lines = ['-- Fictional Inua Mkulima business data; no session data.', 'PRAGMA foreign_keys = ON;', 'BEGIN TRANSACTION;'];
    const tables = ['schema_migrations', 'products', 'wallets', 'transactions', 'transaction_items'];
    for (const table of tables) {
      const schema = db.prepare("SELECT sql FROM sqlite_schema WHERE type = 'table' AND name = ?").get(table) as { sql: string } | undefined;
      if (!schema) continue;
      ...
      const quoted = columns.map((column) => `CASE WHEN typeof("${column}") = 'text' AND instr("${column}", char(0)) > 0 THEN 'CAST(X''' || hex("${column}") || ''' AS TEXT)' ELSE quote("${column}") END AS "${column}"`);
      const rows = db.prepare(`SELECT ${quoted.join(', ')} FROM "${table}" ORDER BY rowid`).all() as Record<string, string>[];
      for (const row of rows) lines.push(`INSERT INTO "${table}" (${columns.map((column) => `"${column}"`).join(', ')}) VALUES (${columns.map((column) => row[column]).join(', ')});`);
    }
    const indexes = db.prepare("SELECT sql FROM sqlite_schema WHERE type = 'index' AND sql IS NOT NULL ORDER BY name").all() as { sql: string }[];
