...
if (!process.env.DATABASE_PATH)
  throw new Error(
    "DATABASE_PATH is required. Configure your local .env before running a database command.",
  );
  ...
  switch (command) {
    ...
    case "reset":
      if (!process.argv.includes("--confirm-demo-reset"))
        throw new Error(
          "Reset requires --confirm-demo-reset; this permanently clears local business data.",
        );
      ...
      db.transaction(() => {
        db.exec(
          "DELETE FROM transaction_items; DELETE FROM transactions; DELETE FROM wallets; DELETE FROM products;",
        );
        seed(db, process.env.DEMO_DEALER_ID ?? "demo-dealer");
      }).immediate();
      break;
    case "dump": {
      ...
      if (target === resolve(databasePath))
        throw new Error("Dump target must differ from the database file.");
    ...
    case "restore":
      ...
      if (
        ...
        throw new Error("Restore requires a fresh, empty database.");
      db.exec(readFileSync(file, "utf8"));
      break;
    default:
      throw new Error(
        "Expected migrate, seed, reset, dump [file], or restore <file>.",
      );
  }
} finally {
  db.close();
