...
describe("authenticated checkout API", () => {
  ...
  it("enforces origin, credentials, authenticated routes, secure cookie attributes and logout", async () => {
    ...
    const second = await client
      ...
      .send({ username: "demo", password })
      .expect(200);
    expect(second.headers["set-cookie"][0].split(";")[0]).not.toBe(
      cookie.split(";")[0],
    );
    await request(f.app).get("/api/auth/me").set("Cookie", cookie).expect(401);
    expect(
      (await client.get("/api/auth/me").expect(200)).body.data.walletId,
    ).toBe(1);
  ...
  it("persists the worked purchase, replays it, returns history and downloads a PDF even if queue wake fails", async () => {
    ...
    expect(
      (await client.get("/api/products").expect(200)).body.data,
    ).toHaveLength(5);
    expect(
      (await client.get("/api/wallet").expect(200)).body.data.balanceMinor,
    ).toBe(240000);
    ...
    expect((await client.get("/api/wallet")).body.data.balanceMinor).toBe(
      240000,
    );
    ...
    expect(
      (await client.get(`/api/transactions/${result.body.data.id}`)).body,
    ).toEqual(result.body);
    const receipt = await client
      .get(`/api/transactions/${result.body.data.id}/receipt`)
      .expect(200)
      .expect("Content-Type", /application\/pdf/);
    expect(receipt.body.subarray(0, 5).toString()).toBe("%PDF-");
    const logs = JSON.stringify(f.events);
    expect(logs).toContain(result.body.data.id);
  ...
  it("validates code/key and stale prices without writing and protects historical snapshots", async () => {
    ...
    await client
      ...
      .send({ ...payment, verificationCode: "000000" })
      .expect(400);
    await client
      .post("/api/transactions")
      .set("Origin", origin)
    ...
    await client
      ...
      .set("Origin", origin)
      .send({ name: "Changed feed", priceMinor: 180000 })
      .expect(200);
    await client
      .post("/api/transactions/preview")
      .set("Origin", origin)
    ...
    await client
      ...
      .set("Origin", origin)
      .set("Idempotency-Key", key)
      .send({ ...payment, items: [...payment.items].reverse() })
      .expect(200);
    const created = await client
      .post("/api/products")
      .set("Origin", origin)
      .send({ name: " New seeds ", priceMinor: 10000 })
      .expect(201);
  ...
  it("rejects session expiry and hides purchases from another dealer", async () => {
    ...
    f.db
      .prepare("UPDATE transactions SET dealer_id = ? WHERE id = ?")
      .run("other-dealer", saved.id);
    await client.get(`/api/transactions/${saved.id}`).expect(404);
    await client.get(`/api/transactions/${saved.id}/receipt`).expect(404);
    expect((await client.get("/api/transactions")).body.data.total).toBe(0);
    f.sessionDb
      .prepare("UPDATE sessions SET expire = '2000-01-01T00:00:00.000Z'")
      .run();
  ...
  it("returns bounded JSON errors, correlation IDs and rate limits without leaking raw input", async () => {
    ...
    expect(malformed.body.error.requestId).toBe(
