...
import {
  ...
  listTransactions,
  previewPurchase,
} from "../src/transactions/service.js";
import {
  cartSchema,
  paymentSchema,
  type CartPayload,
...
describe("persisted purchases and accounting invariants", () => {
  ...
  const balance = () =>
    (
      db
        .prepare(
          "SELECT balance_minor AS balance FROM wallets WHERE dealer_id = ?",
        )
        .get(dealer) as { balance: number }
    ).balance;
  const count = () =>
    (
  ...
  const purchase = (payload = cart(), key = randomUUID()) =>
    completePurchase(db, dealer, payload, key, randomUUID());

  it("A07/A12: previews without writes then saves the exact worked example and pending event", () => {
    expect(previewPurchase(db, dealer, cart())).toMatchObject({
      purchaseTotalMinor: 210000,
      deductionTotalMinor: 140000,
      customerDueMinor: 70000,
      walletBalanceMinor: 240000,
      walletAfterMinor: 100000,
    });
    ...
    expect(count()).toBe(1);
    expect(
      db.prepare("SELECT event_published_at FROM transactions").get(),
    ).toEqual({ event_published_at: null });
  });
  ...
  it("A09: an actual item-insert failure rolls back header, earlier items and debit", () => {
    db.exec(
      "CREATE TRIGGER fail_second_item BEFORE INSERT ON transaction_items WHEN NEW.product_id = 2 BEGIN SELECT RAISE(ABORT, 'injected item failure'); END",
    );
    expect(() => purchase()).toThrow("injected item failure");
    expect(balance()).toBe(240000);
    expect(count()).toBe(0);
    expect(
      db.prepare("SELECT count(*) AS count FROM transaction_items").get(),
    ).toEqual({ count: 0 });
  });
  it("A10: retry after price/deactivation/balance changes returns original snapshot without debit", () => {
    const key = randomUUID();
    const first = purchase(cart(), key);
    db.prepare(
      "UPDATE products SET name = 'Changed', price_minor = 1, active = 0 WHERE id = 1",
    ).run();
    ...
    expect(count()).toBe(1);
    expect(() =>
      purchase({ ...cart(), expectedDeductionTotalMinor: 1 }, key),
  ...
  it("A15: saved parties and item names/prices survive edits; access is owner scoped", async () => {
    ...
    db.exec(
      "UPDATE products SET name = 'Renamed', price_minor = 1; UPDATE wallets SET farmer_name = 'Changed farmer'",
    );
    expect(getTransaction(db, dealer, transaction.id)).toEqual(transaction);
    expect(() => getTransaction(db, "another-dealer", transaction.id)).toThrow(
      "not found",
    );
  ...
  it("A14: a receipt paginates fifty long product names and includes the provided vector logo", async () => {
    ...
    for (let id = 1; id <= 50; id++) {
      const name =
        `Product ${id} ` +
        "Long fictional agricultural product description ".repeat(4);
      db.prepare(
        "INSERT INTO products(id,name,price_minor,created_at,updated_at) VALUES (?,?,100,?,?) ON CONFLICT(id) DO UPDATE SET name = excluded.name, price_minor = 100",
  ...
  it("A19: migration/seed are repeatable and restart plus SQL restore preserve purchases", () => {
    ...
    db.close();
    db = openDatabase(path);
    expect(getTransaction(db, dealer, first.transaction.id)).toEqual(
      first.transaction,
    );
    ...
    try {
      ...
      migrate(restored);
      seed(restored);
      expect(getTransaction(restored, dealer, first.transaction.id)).toEqual(
        first.transaction,
      );
      expect(
        restored.prepare("SELECT balance_minor FROM wallets").get(),
      ).toEqual({ balance_minor: 100000 });
  ...
  it("A19: dump preserves quoted and NUL-containing product names exactly", () => {
    ...
    try {
      restored.exec(dumpDatabase(db));
      expect(
        restored.prepare("SELECT name FROM products WHERE id = 1").get(),
      ).toEqual({ name });
    } finally {
      restored.close();
    }
  ...
  it.each([
    ...
    [
      "negative deduction",
      (value: CartPayload) => {
        value.items[0]!.deductionMinor = -1;
      },
    ],
    [
      "unsafe amount",
      (value: CartPayload) => {
        value.items[0]!.deductionMinor = Number.MAX_SAFE_INTEGER + 1;
      },
    ],
    [
      "zero total",
      (value: CartPayload) => {
        value.expectedDeductionTotalMinor = 0;
      },
    ],
    ...
    expect(() => purchase(value)).toThrow();
    expect(balance()).toBe(240000);
    expect(count()).toBe(0);
  });
  it.each([
    [
      "line deduction above total",
      (value: CartPayload) => {
        value.items[0]!.deductionMinor = 150001;
      },
      "line deduction",
    ],
    [
      "wrong total",
      (value: CartPayload) => {
        value.expectedDeductionTotalMinor = 10;
      },
      "submitted deduction total",
    ],
    [
      "price conflict",
      (value: CartPayload) => {
        value.items[0]!.expectedUnitPriceMinor = 1;
      },
      "price changed",
    ],
    [
    ...
    const value = cart();
    change(value);
    expect(() => purchase(value)).toThrow(message);
    expect(balance()).toBe(240000);
    expect(count()).toBe(0);
  });
  it("A06: rejects inactive products, unsafe line multiplication and unsafe cart sums", () => {
    ...
    db.prepare(
      "UPDATE products SET active = 1, price_minor = ? WHERE id IN (1, 2)",
    ).run(Number.MAX_SAFE_INTEGER);
    const value = cart();
    value.items.forEach((item) => {
      item.expectedUnitPriceMinor = Number.MAX_SAFE_INTEGER;
      item.quantity = 1;
    });
  ...
  it("A12: requires a six-digit payment code at the request boundary", () => {
    ...
    expect(
      paymentSchema.safeParse({ ...cart(), verificationCode: "12345a" })
        .success,
    ).toBe(false);
    expect(
      paymentSchema.safeParse({ ...cart(), verificationCode: "123456" })
        .success,
    ).toBe(true);
  });
    ...
    const launch = () => {
      ...
      const result = new Promise<{ ok: boolean; code?: string }>(
        (resolve, reject) => {
          ...
          child.on("close", (code) => {
            if (code !== 0) {
              reject(new Error(errors || `Child exited ${code}`));
              return;
            }
            try {
      ...
      return {
        ready: Promise.race([
          ready,
          result.then(() => {
            throw new Error("Worker exited before start.");
          }),
        ]),
        result,
        child,
