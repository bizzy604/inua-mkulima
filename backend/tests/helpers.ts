...
export function fixture(onPurchase?: () => void) {
  ...
  const config = parseConfig({
    NODE_ENV: "test",
    APP_ORIGIN: origin,
    DATABASE_PATH: join(dir, "business.db"),
    SESSION_DATABASE_PATH: join(dir, "sessions.db"),
  ...
  const { app, close } = createApp({
    ...
    sessionDb,
    onPurchase,
    logger: { info: (message, meta) => events.push({ message, ...meta }) },
  });
  return {
    app,
    db,
