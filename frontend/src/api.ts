...
export interface PreviewLine extends CartLine {
  productName: string;
...
export interface Transaction extends Preview {
  id: string;
  createdAt: string;
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    error.name = body.error?.code ?? "REQUEST_ERROR";
    throw error;
  }
...
export const api = {
  me: () =>
    request<{ dealerId: string; username: string; walletId: number }>(
      "/auth/me",
    ),
  ...
  preview: (payload: {
    items: CartLine[];
    expectedDeductionTotalMinor: number;
  }) => request<Preview>("/transactions/preview", json(payload)),
  pay: (
    payload: {
      items: CartLine[];
      expectedDeductionTotalMinor: number;
